...
let SupportService = class SupportService {
    ...
    async findOne(SupportID) {
        const support = await this.supportRepository.findOne({ where: { SupportID: SupportID } });
        if (!support) {
            throw new common_1.NotFoundException(`Support ticket with ID ${SupportID} not found`);
        }
        return support;
    }
    async update(SupportID, updateSupportDto) {
