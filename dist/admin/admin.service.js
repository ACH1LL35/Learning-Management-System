...
let AdminService = class AdminService {
    ...
    async create(createAdminDto, userId) {
    ...
    async update(adminId, updateAdminDto, sessionUserId) {
        const admin = await this.adminRepository.findOne({ where: { AdminID: adminId }, relations: ['User'] });
        if (!admin) {
            throw new common_1.BadRequestException(`Admin with ID ${adminId} not found`);
        }
