...
export class ParentController {
  ...
  async create(@Body() createParentDto: ParentDto) {
    ...
    // Only send email if parent was created successfully
    if (createdParent) {
      try {
  ...
  async update(
    ...
    const sessionParentId = parseInt(session.parentId, 10); 
  ...
  private async sendProfileUpdatedEmail(email: string) {
    await this.mailerService.sendMail({
      ...
      text: 'Some of your profile information on our platform was changed.',
    });
  }
}